"""Rebuild canonical continuous-lane-change record.json files from EvidenceChain.csv files."""
...
ROOTS = (
    Path(r"Z:\HongqiData\Nanjing_valid\06_ContinueLaneChange"),
    Path(r"Z:\HongqiData\Changchun_valid\06_ContinueLaneChange"),
)
ROAD_TYPES = {
...
ARTICLE_TEXT = "A vehicle should not change across two or more lanes consecutively."
...
def article_status(rows: list[dict[str, str]]) -> str:
    states: list[str] = []
    for row in rows:
        if number(row.get("trigger_OSP_9_3_1")) == 0:
            continue
        value = int(number(row.get("com_OSP_9_3_1")))
        if value == 1:
            states.append("Compliance")
        elif value == -1:
            states.append("Violation")
    if not states:
        raise ValueError("no continuous-lane-change compliance decision")
...
def build_record(record_path: Path) -> dict:
    ...
    evidence, _ = s7._continuous_lane_change_evidence(rows, trigger_mask)
    for key in ("_compliance_label", "_anchor_type", "_anchor_start_time_s", "_anchor_end_time_s", "Gap_within_prohibited_interval"):
        evidence.pop(key, None)
    evidence["Article_status"] = {"OSP_9.3.1": status}
    ...
    scenario = "The continuous-lane-change evidence was summarized from the recorded trigger interval."
    record = {
    ...
    record.update({
        "Article": {"ID": "OSP_9.3.1", "Text": [ARTICLE_TEXT]},
        "EventAnchor": {
        ...
        "Result": {
            ...
            "Violation_reason": "The ego vehicle changed across lanes consecutively within the prohibited interval." if label == "Violation" else "None",
            "Driving_suggestion": "Avoid consecutive lane changes within the prohibited interval." if label == "Violation" else "Maintain the observed compliant lane-change behavior.",
...
def records(root: Path, segment_glob: str) -> list[Path]:
    return sorted(root.glob(f"{segment_glob}/event_*/ContinueLC_event_*_record.json"))
