...
% ========= 2) 搜索所有 all.mat 以及对应的 ContinueLC_events.csv =========
...
for i = 1:numMats
    ...
    for j = 1:num_event
        ...
        % ---- 写CSV到对应event目录 ----
        % 建议文件名：ContinueLC_event_<n>_EgoInfo.csv
        EgoInfo = ['ContinueLC_event_', num2str(j), '_EgoInfo.csv'];
        csv_path = fullfile(out_dir, EgoInfo);
        writetable(T1, csv_path);
        fprintf('[%d/%d] File "%s" has been written successfully!\n', j, num_event, EgoInfo);

        ObjInfo = ['ContinueLC_event_', num2str(j), '_ObjInfo.csv'];
        csv_path2 = fullfile(out_dir, ObjInfo);
        writetable(T2, csv_path2);
        fprintf('[%d/%d] File "%s" has been written successfully!\n', j, num_event, ObjInfo);

        MapInfo = ['ContinueLC_event_', num2str(j), '_MapInfo.csv'];
