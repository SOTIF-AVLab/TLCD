...
% ========= 2) Find all.mat and corresponding LateralDis_events.csv =========
...
for d = 1:numel(date_dirs)
    ...
    for s = 1:numel(seg_dirs)
        ...
        event_record_path = fullfile(event_record_root, segment_name, 'LateralDis_events.csv');
...
fprintf('Found %d all.mat files with LateralDis_events.csv.\n', numMats);
if numMats == 0
    error('No matched all.mat/LateralDis_events.csv files found. Expected YYYYMMDD\\mat\\<segment>\\all.mat and YYYYMMDD\\zEvent_LateralDis\\<segment>\\LateralDis_events.csv.');
end

% ========= 3) Split each lateral-distance event into input CSV files =========
for i = 1:numMats
